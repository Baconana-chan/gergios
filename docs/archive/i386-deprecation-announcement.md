...
| **Phase 3** | Hard Deprecation | Q2 2026 | ✅ **CURRENT** |
| **Phase 4** | Complete Removal | After ARM64 production readiness | ⏳ Pending |

## What This Means

### For Users on i386 Hardware
- **i386 is Phase 3 hard deprecated** — best-effort community support only
- **Builds require explicit opt-in**: `cmake -DMACHINE_ARCH=i386 -DMKI386=ON`
- **No official support**, no guaranteed fixes, no CI/CD testing
- **Migration deadline**: Phase 4 (complete removal) after ARM64 production readiness
- We strongly recommend migrating to x86_64 or ARM64 hardware immediately
- Migration guides and support resources are available (see below)

### For Developers
- **No new features** for i386 (see [Hard Deprecation Notice](i386-hard-deprecation-notice.md))
- **Community-only maintenance** — best-effort, no guarantees
- **i386 does not build by default** — requires `-DMKI386=ON`
- i386 code preserved in tree but removed from CI/CD
- All new development **must** target x86_64 or ARM64
- Contributions to x86_64 and ARM64 are highly encouraged
...
### Phase 3 (Current) — Hard Deprecation
...
- ✅ Migration deadline set (Phase 4 after ARM64)

### Phase 4 — Removal (Pending ARM64)
- All i386 architecture code removed
- Final archived release
- Git tag `archive/i386-last` created
- Separate branch for community maintenance
