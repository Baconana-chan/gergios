//! # FFI — MINIX C system call bindings for the NVMe driver
//!
//! Dual-platform: real MINIX extern blocks + host stubs for cargo test.
//! Follows the same pattern as minix-ahci/src/ffi.rs.
...
// ============================================================================
// Shared types (used by both MINIX and host builds)
// ============================================================================
...
#[derive(Clone, Copy)]
pub struct IoVec {
    pub iovec_grant: c_int,
    pub iovec_size: usize,
}
...
pub fn pci_write_cfg16_ffi(devind: c_int, offset: c_int, val: u16) {
    unsafe { platform::pci_attr_w16(devind, offset, val); }
}
...
mod tests {
    ...
    #[test]
    fn raw_read_write_64() {
        let mut val: u64 = 0;
        unsafe {
            write64_raw(&mut val as *mut u64 as usize, 0xCAFE_BABE_DEAD_BEEF);
            assert_eq!(read64_raw(&mut val as *mut u64 as usize), 0xCAFE_BABE_DEAD_BEEF);
        }
    }
    ...
    #[test]
    fn io_vec_size() {
        assert_eq!(core::mem::size_of::<IoVec>(), 2 * core::mem::size_of::<usize>());
    }
}
