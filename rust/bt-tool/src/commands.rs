...
pub fn usage(progname: &str) {
  ...
  {} register-service <uuid> <psm> <channel> <name>
  {} -h                  Show this help",
...
pub fn cmd_register_service(uuid_hex: &str, psm_hex: &str, channel_str: &str, name: &str) -> i32 {
    let uuid16 = match u16::from_str_radix(uuid_hex, 16) {
        Ok(v) => v,
        Err(_) => {
            eprintln!("bt-tool: invalid UUID16 '{}'", uuid_hex);
            return 1;
        }
    };
    let psm = match u16::from_str_radix(psm_hex, 16) {
        Ok(v) => v,
        Err(_) => {
            eprintln!("bt-tool: invalid PSM '{}'", psm_hex);
            return 1;
        }
    };
    let channel: u8 = match channel_str.parse() {
        Ok(v) => v,
        Err(_) => {
            eprintln!("bt-tool: invalid channel '{}'", channel_str);
            return 1;
        }
    };
    ...
    msg.write_i32(0, psm as i32);       // BT_REG_PSM
    msg.write_i32(8, channel as i32);   // BT_REG_CHANNEL
    msg.write_i32(16, uuid16 as i32);   // BT_REG_UUID16
    msg.write_i32(24, 0);               // BT_REG_FLAGS

    // Copy name into payload starting at offset 32 (max 24 bytes)
    let name_bytes = name.as_bytes();
    let len = name_bytes.len().min(23);
    msg.payload[32..32 + len].copy_from_slice(&name_bytes[..len]);
    msg.payload[32 + len] = 0;

    let r = bt_sendrec(&mut msg);
    if r == 0 {
        let handle = msg.read_i32(0);
        println!("Service registered with handle {}", handle);
    }
    r
}
