...
static void announce(void)
{
  /* Display the GergiOS startup banner. */
  printf("\n%s %s. "
#ifdef PAE
"(PAE) "
#endif
	...
	"(" _VCS_REVISION ")\n"
      ...
      "Based on MINIX 3.4.0 microkernel\n",
      OS_NAME, OS_RELEASE);
}
...
{
/* This function prepares to shutdown GergiOS. */
   ...
   */
  printf("GergiOS will now be shut down ...\n");
 ...
 */
  ...
  if((how & RB_POWERDOWN) == RB_POWERDOWN)
	direct_print("GergiOS has halted and will now power off.\n");
  else if(how & RB_HALT)
	direct_print("GergiOS has halted. "
		     "It is safe to turn off your computer.\n");
  else
	direct_print("GergiOS will now reset.\n");
  arch_shutdown(how);
}
