...
## 12. Future Ideas — BitTorrent-транспорт для pkgsrc (pkgsync)

> **Статус**: 🔮 Идея — не запланировано, не требует реализации в ближайшее время.
> **Основание**: Обсуждение в чате — BitTorrent как отдельный MINIX-сервер — overkill, но BitTorrent-совместимый транспорт для пакетного менеджера даёт реальную пользу для кластеров и офисов.

### Проблема

При обновлении N машин в кластере или офисе каждая скачивает одни и те же пакеты из интернета (или с одного зеркала). Это:
- Избыточный трафик (каждый пакет скачивается N раз)
- Зависимость от CDN/зеркала (единая точка отказа)
- Медленно в air-gapped сетях (флешка-синхронизация)

### Решение: pkgsync — BitTorrent-транспорт для pkgsrc

**Архитектура**: userspace демон (не MINIX-сервер, не ядерный модуль).

```
pkgsync (userspace демон)
├── BitTorrent-совместимый транспорт (только download)
│   ├── Magnet URI → metadata fetch
│   ├── Piece download с SHA1 верификацией
│   └── DHT для peer discovery (без tracker)
├── Интеграция с pkgsrc/pkg_info
│   ├── Пакеты как .torrent-файлы (metadata + piece hashes)
│   └── Автоматическая публикация установленных пакетов как seed
└── LAN peer discovery
    ├── mDNS/Bonjour для локальной сети
    └── Автоматический peer exchange при pkgin upgrade
```

### Ключевые архитектурные решения

1. **НЕ MINIX-сервер** — не использует привилегированные IPC, не управляет hardware, не требует ring 0. Обычный userspace процесс с сокетами.
2. **Только download** — демон умеет скачивать и раздавать (seed), но не создавать торренты. Публикация новых пакетов — через существующие репозитории (зеркало генерирует .torrent).
3. **DHT-only** — не требует tracker-сервера. Peer discovery через Kademlia DHT + mDNS для LAN.
4. **Прозрачность для пользователя** — `pkgin upgrade` работает как обычно, pkgsync под капотом выбирает: скачать с зеркала (HTTP) или через BitTorrent, в зависимости от наличия пиров.

### Плюсы для сценариев

| Сценарий | Выигрыш |
|----------|---------|
| Кластер из 10 MINIX-серверов | 1× download из интернета, 9× по LAN — **~10× экономия внешнего трафика** |
| Офис с 5 MINIX-машинами | Обновления не нагружают внешний канал |
| Air-gapped сеть | Единственный практичный способ синхронизации без флешек |
| Одиночная машина | Нет проигрыша — fallback на HTTP |

### Оценка сложности

| Компонент | LOC | Сложность |
|-----------|-----|-----------|
| **Торрент-транспорт** (wire protocol, piece mgmt, magnet, DHT peer) | ~3,000 | Medium |
| **Интеграция с pkgsrc** (meta-пакеты, хук в pkgin) | ~1,000 | Low |
| **LAN discovery** (mDNS) | ~500 | Low |
| **Итого** | **~4,500** | |

Это ~15-25% от полноценного BitTorrent-клиента (который требует DHT, PEX, µTP, Magnet, Merkle tree, piece selection, disk scheduler).

### Когда имеет смысл

- **Phase 7+** — после стабильной сетевой подсистемы (lwIP, inet)
- **Приоритет**: P4 (ниже NVMe, xHCI, HDA, Driver Manager)
- **Триггер**: Появление реального кластера MINIX/GergiOS машин

### Минусы (почему НЕ делать сейчас)

1. BitTorrent wire protocol — не тривиальный (bencode, handshake, piece negotiation)
2. DHT (Kademlia) — распределённая система с race conditions
3. Пакетный менеджер pkgsrc не имеет hooks для альтернативных транспортов
4. Низкий приоритет: пока нет кластера, пользы нет

---

## 13. Рекомендация: Что можно делать без QEMU
